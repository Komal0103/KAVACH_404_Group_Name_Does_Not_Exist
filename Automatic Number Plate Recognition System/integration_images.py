...
def model2_process_frame(frame):
    ...
    cv2.imshow('face detection',  cv2.resize(frame, (800, 600)))
    cv2.waitKey(0)

def main():
    IMAGE_PATH = os.path.join(paths['IMAGE_PATH'], 'test','TestImages' , '1.jpg')                  
        # detection happens here
    frame = cv2.imread(IMAGE_PATH)

    # Start model 1 worker thread
    model1_thread = threading.Thread(target=model1_process_frame, args=(frame,))
    model1_thread.start()

    # Start model 2 worker thread
    model2_thread = threading.Thread(target=model2_process_frame, args=(frame,))
    model2_thread.start()
